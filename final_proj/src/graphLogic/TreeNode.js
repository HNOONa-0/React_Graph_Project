// const makechildren = (parent, children, idx, adjacencyList) =>{
//     adjacencyList[idx].forEach(e => children.push(new TreeNode(parent, e, adjacencyList)))
// }
// makechildren(this, this.children, idx, adjacencyList);
// assuming graph is directed and non-cyclic
// this.children = [...adjacencyList[idx]].map(e => new TreeNode(this, e, adjacencyList)); works
const rootSize = 1;
...
export class TreeNode{
    ...
    constructor(parent, idx, depth, adjacencyList){
        ...
        this.parent = parent;
        this.idx = idx;
        this.children = [...adjacencyList[idx]].map(e => new TreeNode(this, e, depth + 1, adjacencyList));
        this.leftSibling = 0;
        this.rightSibling = 0;
        this.x = 0;
        this.y = depth;
    ...
    static preorder = (root, mp) =>{
        ...
        for (let i = 0; i < root.children.length; i++) this.preorder(root.children[i], mp);
    }
    ...
    static calculateInitialX(root){
        ...
        if (isLeaf(root))
        {
            ...
            if (!isLeftMost(root)) root.x = getPreviousSibling(root).x + rootSize + siblingDistance;
            else root.x = 0;
        }
    ...
    static checkForConflicts(root){
        ...
        while (sibling && sibling !== root)
        {
            ...
            for (let level = root.y + 1; 
                level <= Math.min(Math.max(...rootContour.keys()), Math.max(...siblingContour.keys())); level++)
            {
    ...
    static centerNodesBetween(leftRoot, rightRoot){
        ...
        {
            let distanceBetweenNodes = (leftRoot.x - rightRoot.x) / (numNodesBetween + 1);
    ...
    static getLeftContour(root, modSum, values){
        if (!values.has(root.y))
            values.set(root.y, root.x + modSum);
        else
            values[root.y] = Math.min(values[root.y], root.x + modSum);

        modSum += root.mod;
        for (let i = 0; i < root.children.length; i++) this.getLeftContour(root.children[i], modSum, values);
    }
    static getRightContour(root, modSum, values){
        if (!values.has(root.y))
            values.set(root.y, root.x + modSum);
        else
            values[root.y] = Math.max(values[root.y], root.x + modSum);

        modSum += root.mod;
        for (let i = 0; i < root.children.length; i++) this.getRightContour(root.children[i], modSum, values);
    }
}
...
//     this.rightSibling = 0;
// }
// static intializeSiblings = (root, curIdx = 0) =>{
//     // intialize siblings using a preorder traversal
...
//     else{
//         if (curIdx - 1 >= 0){
//             root.leftSibling = children[curIdx - 1];
//         }
//         if (curIdx + 1 < children.length){
//             root.rightSibling = children[curIdx + 1];
//         }
//     }
//     for (let i = 0; i < root.children.length; i++) this.intializeSiblings(root.children[i], i);
// }
// static firstWalk = (root)=>{
//     // a postorder traversal
//     // if its a leaf -> give it next x
//     if (root.isLeaf()){
//         if (root.isLeftMost()) root.x = 0;
//         else root.x = root.leftSibling.x + 1;
//     }
// }
// static buildTree = (adjacencyList) =>{
//     // make a tree & intialize left, right siblings
//     let root = new TreeNode(0, 1, 0, adjacencyList);
//     this.intializeSiblings(root);
